...
Reviewer code / 검수자 코드: **KO-R2-Q73JV**   Language / 언어: **Korean (한국어)**   Items / 문항: **200**
...
## 1. Item judgments / 문항 판정 (`items-KO-R2-Q73JV.csv`)

Each row shows two sentences in Korean (한국어). Both may be wrapped in the same frame (a question, reported speech, or a
...
- `noun-forms-ko.csv`: every noun phrase surface form (case, number). Mark errors by row.
  모든 명사구 표면형(격·수). 오류가 있는 행을 표시.
- `construction-examples-ko.csv`: one example per construction. Check grammaticality and naturalness.
  구문별 대표 문장. 문법성·자연스러움 확인.
- `template-inventory.json`: vocabulary tables. 어휘 표.
- `template-checklist-KO-R2-Q73JV.csv`: record `checked` (yes/no), `issue` (yes/no) and comments per row.

## 3. Return / 제출

Return `items-KO-R2-Q73JV.csv` and `template-checklist-KO-R2-Q73JV.csv` to the operator. Keep a copy.
