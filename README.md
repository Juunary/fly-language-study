...
영어·독일어·한국어의 네 의미 과제와 여섯 학습 순서를 비교하는 실행 패키지다. **현재는 구현·CPU 검증 단계이며, GPU 학습 결과가 아니다.**
...
이 PC에는 Intel Arc GPU가 있다. RTX 3080 Ti 두 장의 CUDA 결과, 처리량, 숙달 여부는 아직 측정하지 않았다. `reports/g0-local.json`은 **blocked**가 정상이다. CUDA 검사가 건너뛰어졌다는 이유로 통과시키지 않는다.

문항은 생성된 **초안**이다. 각 언어의 능숙자 두 명이 검수하기 전에는 G2·G3·순서 파일럿·본실험을 열지 않는다. G0·처리량·G1은 이와 독립적인 기술 검증으로 먼저 할 수 있다. G1 통과는 일반화의 증거가 아니다.
...
| 경로 | 용도 |
|---|---|
| `configs/protocol-v4.json` | 기본 후보: N=20, 평가 간격 10,240. 아직 확정 아님 |
...
5. 실제 사람의 검수 자료로 `certify-review`, 이어서 `pilot-ready`를 실행한다.
...
조건문·인용·질문 틀의 자연스러움과 의미 일치 정의는 사람이 확인해야 한다. 단서 기준선 통과는 모든 지름길이 제거됐다는 증거가 아니다. 이 실험은 살아 있는 초파리, CEFR, 언어의 보편적 난이도를 측정하지 않는다.
